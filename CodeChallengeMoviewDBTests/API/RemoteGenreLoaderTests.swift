...
class RemoteGenreLoaderTests: XCTestCase {
    ...
    func test_load_returnsItemsOn200HTTPResponseWithJSONItems() {
        let (sut, clientSpy) = makeSut()
        let genreRoot = GenreHelper.makeGenreRoot()
        expect(sut, toCompleteWith: .success(genreRoot)) {
            let jsonData = JSONHelper.loadJsonData(for: self, andResourceName: "genreData")
    ...
    func test_assertThereIsNoCallAfterSUTHasBeenDeallocated() {
        ...
        let url = URL(string: "http://any-url.com")!
        var sut: RemoteGenreLoader? = RemoteGenreLoader(client: client)
        var results = [RemoteGenreLoader.Result]()
    ...
    private func expect(_ sut: RemoteGenreLoader,
                toCompleteWith expectedResult: GenreLoader.Result,
    ...
    private func makeSut(url: URL = URL(string: "http://any-url.com")!) -> (RemoteGenreLoader, HTTPClientSpy) {
        let clientSpy = HTTPClientSpy()
        let sut = RemoteGenreLoader(client: clientSpy)
